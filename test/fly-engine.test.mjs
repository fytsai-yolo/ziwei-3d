...
ok('祿隨忌走: chain from 生年祿 palace follows 宮干忌 hops, detects the cycle', () => {
  const lsj = chart.flying.luSuiJi;
  assert.equal(lsj.luStar, '天機');
  assert.equal(lsj.startIndex, byBranch('巳').branchIndex); // 天機祿坐福德(巳)
  // 福德 →忌(文昌)→ 命宮 →忌(文曲)→ 財帛 →忌(巨門)→ 夫妻 →忌(文曲)→ 財帛(revisit)
  assert.deepEqual(lsj.steps.map(s => [s.to, s.star]), [
    [byBranch('卯').branchIndex, '文昌'],
    [byBranch('亥').branchIndex, '文曲'],
    [byBranch('丑').branchIndex, '巨門'],
    [byBranch('亥').branchIndex, '文曲'],
  ]);
  assert.equal(lsj.cycle, true);
  assert.equal(lsj.endIndex, byBranch('亥').branchIndex);
  // Each step must genuinely follow the flying map
  let cur = lsj.startIndex;
  for (const s of lsj.steps) {
    assert.equal(s.from, cur);
    assert.equal(chart.flying.map[cur]['忌'].toIndex, s.to);
    cur = s.to;
  }
});

ok('小限 comes straight from iztro for the target year', () => {
  assert.equal(chart.meta.xiaoXianIndex, byBranch('申').branchIndex); // 2026 小限在僕役(申)
  assert.equal(chart.meta.xiaoXianNominalAge, 32);
});
